...
class DestroyCommandTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $DestroyCommand = $this->getMock('\DigitalOcean\CLI\Domains\DestroyCommand', array('getDigitalOcean'));
        $DestroyCommand
            ->expects($this->any())
            ->method('getDigitalOcean')
            ->will($this->returnValue($this->getMockDigitalOcean('domains', $this->getMockDomains('destroy', $result))));

        $this->application->add($DestroyCommand);

        $this->command = $this->application->find('domains:destroy');
