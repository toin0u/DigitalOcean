...
use DigitalOcean\CLI\Droplets\RebootCommand;
...
class RebootCommandTest extends TestCase
{
    ...
    protected function setUp()
    {
        ...
        $RebootCommand = $this->getMock('\DigitalOcean\CLI\Droplets\RebootCommand', array('getDigitalOcean'));
        $RebootCommand
            ->expects($this->any())
            ->method('getDigitalOcean')
            ->will($this->returnValue($this->getMockDigitalOcean('droplets', $this->getMockDroplets('reboot', $result))));

        $this->application->add($RebootCommand);

        $this->command = $this->application->find('droplets:reboot');
